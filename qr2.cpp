...
int generate_check(qr_comp & original_qr_comp, vector<vector<int>> qr, vector<vector<vector<int>>> & valid_qrs, int & counter, int row, int col, int level){
    ...
    while((next = nextCell(original_qr_comp.n, next[0], next[1], qr))[0] != original_qr_comp.n){
        ...
        pre_proccess(copy_qr_comp, copy_qr);
...
void readQrComp(qr_comp & qr_comp){
    int value;
    ...
    //Read the amount of blacks in each diagonal - main  and antidiagonal
    for(int i = 0; i < 2; i++){
        cin >> value;
...
void printOutput(vector<vector<vector<int>>> & valid_qrs, int n, int counter){
    if (counter == 1) {
        cout << "VALID: 1 QR Code generated!" << endl;
        //print qr code
        print_qr(valid_qrs[0], n);
    }else if(counter > 1){
...
int processQrComp(qr_comp qr_comp, vector<vector<int>> qr, vector<vector<vector<int>>> & valid_qrs, int row, int col, int level){
    int counter = 0;
    int cellsFilled = 0;
    int cellsFilledPreprocessing = 0;

    if(row != -1) { //todos menos p o primeiro caso
        fillCell(qr, qr_comp, row, col, 1, cellsFilled);
        
    } else { //primeiro caso
        cellsFilledPreprocessing = pre_proccess(qr_comp, qr);
    }
    ...
    //Preprocess the qr code
    // int cellsFilledPreprocessing = pre_proccess(qr_comp, qr);
    if(cellsFilledPreprocessing == -1) {//significa q a configuracao atual é invalida
        return counter;
        #ifdef DEBUG
        cout << "Invalid (detected on preprocessing)" << endl;
        #endif
    }
    cellsFilled += cellsFilledPreprocessing;
    ...
    //Generate the valid descendants
    vector<int> next{row, col};
    while((next = nextCell(qr_comp.n, next[0], next[1], qr))[0] != qr_comp.n){
        //fillCell(qr, qr_comp, next[0], next[1], 0, cellsFilled);
        if(!isValidDescendant(qr_comp, next[0], next[1])) {continue; cout << "Invalid descendant level: " << level << endl;}
        #ifdef DEBUG
        cout << "Generated descendant" << endl;
        #endif
        counter += processQrComp(qr_comp, qr, valid_qrs, next[0], next[1], level + 1);
...
int main(){
    ...
    while(n_qr != 0){
        ...
        //Create the qr code and the valid ones container
        vector<vector<int>> qr(qr_comp.n, vector<int>(qr_comp.n, -1));
        vector<vector<vector<int>>> valid_qrs;
        int counter = 0;

        //Generate the valid qr codes
        counter = processQrComp(qr_comp, qr, valid_qrs, -1, qr_comp.n - 1, 0);

        //Print the output
        printOutput(valid_qrs, qr_comp.n, counter);
